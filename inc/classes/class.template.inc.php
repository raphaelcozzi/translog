	...
	*/
	class Template
	{
		...
		function set_block($parent, $handle, $name = '')
		{
			...
			//CHANGES MADE TO SUPPRESS THE PREG_MATCH_ALL ERRORS			
			$pos_start = strpos($str , "<!-- BEGIN $handle -->") + strlen("<!-- BEGIN $handle -->");
			$pos_end = strpos($str , "<!-- END $handle -->");// - strlen("<!-- END $handle -->") + 1;
			$html = substr($str , $pos_start , $pos_end - $pos_start);
			$this->set_var($handle, $html);
			//$this->set_var($handle, $m[1][0]);

			
			
//			echo $parent.' - '.$handle.' - '.$pos_start.'-'.$pos_end.'<br>';
